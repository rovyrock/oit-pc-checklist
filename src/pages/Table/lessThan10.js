...
import { cvmList, TableDataLessThan10 } from '@/data';
...
export default (props) => {
  ...
  const getData = async () => {
    ...
    await sleep(3000).then(() => {
      ...
      setData(TableDataLessThan10.data.list);
      setTotal(TableDataLessThan10.data.total);
      if (TableDataLessThan10.data.list.length === 0) setStatus('empty');
    });
  };
  useEffect(() => {
    // 请求数据
    ...
    <Layout>
      <Body>
        <Content className="oit-layout--fixheader">
          <Content.Header title="Table 数据小于1页" />
          <Content.Body full>
